...
const phase2Data = auditData?.phases?.phase2;

if (phase2Data?.exists) {
  // Documentation exists, check consistency
  executeSmartSkip(phase2Data);
} else {
...
**If `phase2Data.consistencyScore >= 95`:**

```
---
✅ DATA ARCHITECTURE ALREADY DOCUMENTED

File: docs/data-model.md
Consistency: 98%
Status: All entities match schema

Documented Entities (8):
...
✅ PostTag (postId, tagId) - Many-to-many join
...
Data Patterns:
✅ Soft deletes configured (deleted_at)
...
**If user selects Y:**
```
✅ Phase 2 skipped - using docs/data-model.md
...
**If user selects n:**
```
Proceeding with full Phase 2 questionnaire...
...
**If `phase2Data.consistencyScore >= 80 && < 95`:**

```
---
📚 DATA ARCHITECTURE PARTIALLY DOCUMENTED

File: docs/data-model.md
Consistency: 87%
Status: Most entities documented, some gaps detected

Documented (6/8 entities):
✅ User, Post, Comment, Category, Tag, PostTag

Missing in Documentation:
❌ Like (detected in schema but not documented)
❌ Follow (detected in schema but not documented)

Documented Relationships:
✅ User → Post, Post → Comment, Post ↔ Tag
...
Options:
A) Add missing entities only (2 questions) ⭐
B) Re-document everything (full phase)
...
Confirm this structure? (Y/n)
> Y

✅ Like entity added to documentation

---
Question 2/2: Entity 'Follow'
...
This appears to be a self-referential many-to-many relationship.
Confirm? (Y/n)
> Y

✅ Follow entity added to documentation
✅ Phase 2 complete - merged with existing docs/data-model.md
...
**If user selects B:**
```
[Continue to section 2.1 - Full Phase]
```

**If user selects C:**
```
⚠️ Warning: Documentation has gaps but will be used as-is
✅ Proceeding to Phase 3
...
**If `phase2Data.consistencyScore < 80 || !phase2Data.exists`:**

```
---
⚠️ DATA ARCHITECTURE NEEDS DOCUMENTATION

Reason: docs/data-model.md is outdated or missing
Consistency: 62% (or N/A if file doesn't exist)

Issues detected:
❌ Documented entities don't match schema
❌ Missing relationships
❌ Outdated field definitions

Recommendation: Complete full Phase 2 questionnaire
